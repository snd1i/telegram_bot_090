# Dil veritabanı
DILLER = {
    'ku_badini': {
        'code': 'ku_badini',
        'name': 'Kurdî (Badînî) 🇹🇯',
        # Hoşgeldin mesajı için
        'welcome_title': '🌟 خێرهاتی بەرێز {name}!',
        'welcome_line1': 'کەیڤ خوشن ب هاتنا هەوە.',
        'welcome_line2': 'د نافا بوتی دا نەب تنێ وێنە بەلکو چاوانیا دروست کرنا وان وێنا بەلاڤ دکن او نفیسینا وێنە پێ دهێن چێکرن. 🎨',
        'welcome_line3': '🎨 چاوا دێ بکار ینی',
        'welcome_line4': 'او نفیسینا دبن وێنادا "پرومت" دێ کوپی کی.',
        'welcome_line5': 'او ژێریا دەستکرد یاتو بکار دینی بو نمونە (gemini / chatgpt) ێک ژفان بکار بینە.',
        'welcome_line6': 'ستایلێ خو چێکە!',
        'welcome_line7': 'داخازا دەمێت خوش 🚀',
        'welcome_line8': 'بو بدەستفە ینانێ دکما ل خارێ کلیک بکە 👇',
        # Yeni /help mesajı için
        'help_greeting': 'سلاڤ خێرهاتی بەرێز {name} 👋',
        'help_info_title': 'هندەک زانیاری سەبارت بوتی 👇',
        'help_bot_for': 'بوت تایبەت بو کودێن وێنایە 🤖',
        'help_prompts_info': 'کودێن حازرن تنێ دێ کوپی کی و بکار ینی 📋',
        'help_commands_title': 'تایبەت مەندی 📌',
        'help_start_cmd': '/start - بو کار کرنا بوتی',
        'help_help_cmd': '/help - ژبو هاریکاریێ',
        'help_prompts_access': 'بو بدەستفە ینانا کودا تبلا خول کودێن وێنا یان پرومت بدە 🔥',
        'help_more_info': 'بو بدەستفە ینانا پتر زانیاریا تبلا خول دکمێن خارێ بدە ℹ️',
        'help_command': 'ژبو هاریکاریێ',
        # Butonlar
        'button_channel': 'کەنال 📢',
        'button_prompts': 'كودێن وێنا 🔥',
        'button_support': 'هاریکاری 🤝',
        'channel_url': 'https://t.me/sndiyi',
        'prompts_url': 'https://t.me/PrompttAI_bot/Prompts',
        'support_url': 'https://t.me/k4miran_sndi'
    },
    'ku_sorani': {
        'code': 'ku_sorani',
        'name': 'کوردی (سۆرانی) 🇹🇯',
        # Hoşgeldin mesajı için
        'welcome_title': '🌟 بەخێربێیت بۆ {name}!',
        'welcome_line1': 'زۆر دڵخۆشین کە لێرە چاوی پێت دەکەوین.',
        'welcome_line2': 'لەم بۆتەدا تەنها وێنە ناگەیێنین، بەڵکو "نهێنییەکانی" پشتی وێنەکان واتە پرۆمپتەکانیش هاوبەش دەکەین. 🎨',
        'welcome_line3': '🎨 چۆن دەستپێبکەم؟',
        'welcome_line4': 'پرۆمپتەکەی خوارەوەی وێنە کۆپی بکە.',
        'welcome_line5': 'لە ئامرازی هەوەڵی خۆتدا (gemini / chatgpt) تاقی بکەوە.',
        'welcome_line6': 'ستایلەکەی خۆت دروست بکە!',
        'welcome_line7': 'گەڕانێکی خۆش! 🚀',
        'welcome_line8': 'بۆ دەستگەیشتن کرتە لە دووگمەکانی خوارەوە بکە 👇',
        # Yeni /help mesajı için
        'help_greeting': 'سڵاو بەخێربێیت {name} 👋',
        'help_info_title': 'زانیاری دەربارەی بۆتەکەمان 👇',
        'help_bot_for': 'بۆتەکە بۆ پرۆمپتەکانی وێنەیە 🤖',
        'help_prompts_info': 'پرۆمپتە ئامادەکان تەنها کۆپی و پیست 📋',
        'help_commands_title': 'فەرمانەکان 📌',
        'help_start_cmd': '/start - بۆ کردنی بۆت',
        'help_help_cmd': '/help - بۆ یارمەتی',
        'help_prompts_access': 'بۆ دەستگەیشتن بە پرۆمپتەکان کرتە لە دوگمەی پرۆمپت بکە 🔥',
        'help_more_info': 'بۆ زانیاری زیاتر کرتە لە دوگمەکانی خوارەوە بکە ℹ️',
        'help_command': 'بۆ یارمەتی',
        # Butonlar
        'button_channel': 'کەناڵ 📢',
        'button_prompts': 'پرۆمپتەکان 🔥',
        'button_support': 'یارمەتی 🤝',
        'channel_url': 'https://t.me/sndiyi',
        'prompts_url': 'https://t.me/PrompttAI_bot/Prompts',
        'support_url': 'https://t.me/k4miran_sndi'
    },
    'tr': {
        'code': 'tr',
        'name': 'Türkçe 🇹🇷',
        # Hoşgeldin mesajı için
        'welcome_title': '🌟 Aramıza Hoş Geldin {name}!',
        'welcome_line1': 'Seninle burada buluştuğumuz için çok mutluyuz.',
        'welcome_line2': 'Botumuzda sadece resim paylaşmıyoruz, aynı zamanda bu resimlerin arkasındaki "sırları" yani promptları da seninle bölüşüyoruz. 🎨',
        'welcome_line3': '🎨 Nasıl Başlarım?',
        'welcome_line4': 'Paylaşılan resimlerin altındaki "Prompt" kısmını kopyala.',
        'welcome_line5': 'Kendi favori AI aracında (gemini / chatgpt) dene.',
        'welcome_line6': 'Kendi tarzını yarat!',
        'welcome_line7': 'Keyifli keşifler! 🚀',
        'welcome_line8': 'erişmek için aşağıdaki butonlara tıklayın 👇',
        # Yeni /help mesajı için
        'help_greeting': 'Merhaba hoşgeldiniz {name} 👋',
        'help_info_title': 'Botumuz hakkında bilgiler 👇',
        'help_bot_for': 'Bot promptslar içindir 🤖',
        'help_prompts_info': 'Hazır promptlar sadece kopyala yapıştır 📋',
        'help_commands_title': 'Komutlar 📌',
        'help_start_cmd': '/start - Botu çalıştırmak için',
        'help_help_cmd': '/help - Yardım için',
        'help_prompts_access': 'Promptlara erişmek için prompts butonuna tıklayın 🔥',
        'help_more_info': 'Daha fazla bilgi için aşağıdaki butonlara tıklayın ℹ️',
        'help_command': 'Yardım için',
        # Butonlar
        'button_channel': 'Kanal 📢',
        'button_prompts': 'Prompts 🔥',
        'button_support': 'Destek 🤝',
        'channel_url': 'https://t.me/sndiyi',
        'prompts_url': 'https://t.me/PrompttAI_bot/Prompts',
        'support_url': 'https://t.me/k4miran_sndi'
    },
    'en': {
        'code': 'en',
        'name': 'English 🇬🇧',
        # Hoşgeldin mesajı için
        'welcome_title': '🌟 Welcome {name}!',
        'welcome_line1': 'We are very happy to meet you here.',
        'welcome_line2': 'In our bot, we not only share images but also share the "secrets" behind these images, i.e., the prompts. 🎨',
        'welcome_line3': '🎨 How Do I Start?',
        'welcome_line4': 'Copy the "Prompt" section under the shared images.',
        'welcome_line5': 'Try it in your favorite AI tool (gemini / chatgpt).',
        'welcome_line6': 'Create your own style!',
        'welcome_line7': 'Happy exploring! 🚀',
        'welcome_line8': 'click the buttons below to access 👇',
        # Yeni /help mesajı için
        'help_greeting': 'Hello welcome {name} 👋',
        'help_info_title': 'Information about our bot 👇',
        'help_bot_for': 'Bot is for image prompts 🤖',
        'help_prompts_info': 'Ready prompts just copy and paste 📋',
        'help_commands_title': 'Commands 📌',
        'help_start_cmd': '/start - To start the bot',
        'help_help_cmd': '/help - For help',
        'help_prompts_access': 'Click prompts button to access prompts 🔥',
        'help_more_info': 'Click buttons below for more information ℹ️',
        'help_command': 'For help',
        # Butonlar
        'button_channel': 'Channel 📢',
        'button_prompts': 'Prompts 🔥',
        'button_support': 'Support 🤝',
        'channel_url': 'https://t.me/sndiyi',
        'prompts_url': 'https://t.me/PrompttAI_bot/Prompts',
        'support_url': 'https://t.me/k4miran_sndi'
    },
    'ar': {
        'code': 'ar',
        'name': 'العربية 🇮🇶',
        # Hoşgeldin mesajı için
        'welcome_title': '🌟 أهلاً وسهلاً {name}!',
        'welcome_line1': 'نحن سعداء جدًا بلقائك هنا.',
        'welcome_line2': 'في بوتنا، لا نشارك الصور فقط، بل نشارك أيضًا "أسرار" هذه الصور، أي البرومبتات. 🎨',
        'welcome_line3': '🎨 كيف أبدأ؟',
        'welcome_line4': 'انسخ قسم "البرومبت" تحت الصور المشتركة.',
        'welcome_line5': 'جربه في أداتك المفضلة للذكاء الاصطناعي (gemini / chatgpt).',
        'welcome_line6': 'أنشئ أسلوبك الخاص!',
        'welcome_line7': 'استكشاف سعيد! 🚀',
        'welcome_line8': 'انقر على الأزرار أدناه للوصول 👇',
        # Yeni /help mesajı için
        'help_greeting': 'مرحباً أهلاً وسهلاً {name} 👋',
        'help_info_title': 'معلومات عن بوتنا 👇',
        'help_bot_for': 'البوت مخصص لبرومبتات الصور 🤖',
        'help_prompts_info': 'برومبتات جاهزة فقط انسخ والصق 📋',
        'help_commands_title': 'الأوامر 📌',
        'help_start_cmd': '/start - لتشغيل البوت',
        'help_help_cmd': '/help - للمساعدة',
        'help_prompts_access': 'انقر على زر البرومبتات للوصول إليها 🔥',
        'help_more_info': 'انقر على الأزرار أدناه للمزيد من المعلومات ℹ️',
        'help_command': 'للمساعدة',
        # Butonlar
        'button_channel': 'القناة 📢',
        'button_prompts': 'البرومبتات 🔥',
        'button_support': 'الدعم 🤝',
        'channel_url': 'https://t.me/sndiyi',
        'prompts_url': 'https://t.me/PrompttAI_bot/Prompts',
        'support_url': 'https://t.me/k4miran_sndi'
    }
}

# Kullanıcı dil tercihlerini sakla
user_languages = {}

def get_user_language(user_id):
    """Kullanıcının dil tercihini getir"""
    return user_languages.get(user_id, None)

def set_user_language(user_id, lang_code):
    """Kullanıcı dil tercihini kaydet"""
    if lang_code in DILLER:
        user_languages[user_id] = lang_code
        return True
    return False

def get_language_data(user_id):
    """Kullanıcı diline göre dil verilerini getir"""
    lang_code = get_user_language(user_id)
    
    # Eğer dil tercihi yoksa, Türkçe varsayılan
    if not lang_code:
        lang_code = 'tr'
    
    return DILLER.get(lang_code, DILLER['tr'])

def format_user_name(user):
    """Kullanıcı adını formatla"""
    if user.username:
        return f"@{user.username}"
    elif user.first_name:
        return user.first_name
    elif user.last_name:
        return user.last_name
    else:
        lang_code = get_user_language(user.id)
        if lang_code == 'ku_badini':
            return "هەڤال"
        elif lang_code == 'ku_sorani':
            return "هاوڕێ"
        elif lang_code == 'ar':
            return "صديق"
        elif lang_code == 'en':
            return "Friend"
        else:
            return "Arkadaş"
